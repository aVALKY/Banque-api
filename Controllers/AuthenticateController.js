...
class AutenticateController{
    ...
    async login(request, result){
        try {
            ...
            result.status(401)
            result.json({error : "Mot de passe ou email incrorrect"})
        }
    }

    autenticateToken(request, result, next){
        const authHeader = request.headers['authorization'];
        const token = authHeader && authHeader.split(' ')[1];

        if(!token){
            result.status(401);
            return result.json({error : "Vous n'avez pas accès a cette route"})
        }
...
module.exports = new AutenticateController();